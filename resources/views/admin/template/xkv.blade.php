                ...
                <div class="btn-group">
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu">
                            ...
                            <li class="{{$item->id == $model->id ? 'bg-info':''}}"><a href="./{{$item->id}}"><b>{{@substr($item->start_at, 0)}}</b> (<small>{{@substr($item->duration, 0)}}</small>) {{$item->name}} <small class="text-{{$model->status==\App\Models\Template::STATUS_SYNCING?'danger':'info'}}">{{@\App\Models\Template::STATUSES[$model->status]}}</small></a></li>
                            @endforeach
                        </ul>
                    </div>
                </div>
