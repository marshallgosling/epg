...
class XkcController extends AdminController
{
    ...
    {
        $templates = Template::with('records')->where('group_id', $this->group)->where('status', Template::STATUS_SYNCING)->orderBy('sort', 'asc')->get();
        $group = $this->group;
        $data = [];
        $colors = [];
            ...
            if($programs)foreach($programs as $p)
            {
                ...
                if($p->data != null) {
                    ...
                    if(count($p->data['dayofweek']) == 7) $days[] = __('全天');
                        ...
                        foreach($p->data['dayofweek'] as $d) $days[] = __(TemplateRecords::DAYS[$d]);
                    $item = [ $p->id, $p->sort, $p->name, $p->category, TemplateRecords::TYPES[$p->type], $p->data['episodes'], $p->data['date_from'].'/'.$p->data['date_to'], implode(',', $days), $p->data['name'], $p->data['result'], '<a href="xkc/programs/'.$p->id.'/edit">编辑</a>'];
                    if($p->data['result'] == '编排完' || $p->data['result'] == '错误') $style = 'bg-danger';
                }
                else {
                    $item = [ $p->id, $p->sort, $p->name, $p->category, TemplateRecords::TYPES[$p->type], '', '', '', '', '', '<a href="xkc/programs/'.$p->id.'/edit">编辑</a>' ];
                }
                $items[] = compact('style', 'item');
            }

            return new MyTable(['ID', '排序', '别名', '标签', '类型', '剧集', '日期范围', '播出日', '当前选集', '状态', '操作'], $items, ['table-hover']);
        });
