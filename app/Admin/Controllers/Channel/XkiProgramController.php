    ...
    protected function grid()
    {
        ...
        $grid->filter(function(Grid\Filter $filter){
            $filter->column(6, function (Grid\Filter $filter) {
                $filter->equal('channel_id', __('Air date'))->select(Channel::where('name', 'xki')->orderBy('air_date', 'desc')->limit(300)->get()->pluck('air_date', 'id'));
            });
        });
...
TMP;
        ...
        $form->action(admin_url("channel/xki/$id/edit"));
...
*/
        ...
        \App\Tools\Operation::log('保存节目编单', 'channel/XkiProgramController@save', 'post', 
